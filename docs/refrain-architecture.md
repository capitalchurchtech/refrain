...
The setup screen has a **Detect ProPresenter** button (`server/propresenter-scan.js`) so the host and port don't have to be known. On macOS it asks `lsof` which ports ProPresenter is listening on and confirms which one answers the API (`/v1/status/layers`), which handles the common same machine case instantly with no port guessing. If nothing's found locally it scans each private /24 on a couple of common ports (checking the TCP port is open before the HTTP confirm so empty addresses don't each cost a timeout). Found instances are confirmed by the API, so the connection is already known good when the fields are filled in.
