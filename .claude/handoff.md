...
### 5. Texture — same task, item 1. BLOCKED, and harder than before
...
**Blocked twice over.** The candidate tiles are gone — `~/Downloads` now holds
one file and no images at all, so `black_linen_v2.png`, `noisy_net.png`,
`real_cf.png`, `dark_leather.png`, `denim.webp` and `fake_brick.png` are not
recoverable from there. Confirmed by both sessions independently.

So this needs the files back *and* licence provenance for each one before any
tile is inlined. This ships open source; a CC-BY tile with no attribution is a
real problem, not a footnote. **Waiting on Brandon** — do not spec further
texture work until he says where the tiles went.

### 5b. Tailwind is a runtime JIT — see CLAUDE.md

Vendoring solved the offline problem but kept the runtime one:
`public/vendor/tailwind-cdn.js` is 407KB of browser JIT and none of the
vendored CSS contains `.w-36` or `.w-16`. Utilities are still generated by
scanning the DOM, so a class that exists only as a JS string has no rule until
the observer catches it.

That is the real cause of the rail-width puzzle, not specificity. The fix
already in place (`#nav-rail.w-36` and `#main-content.ml-36` owned by
`refrain.css`) is correct and must not be tidied away.

Remaining exposure: `image-crop.js:113-114` toggles `opacity-50` and
`pointer-events-none` from JS strings. The second failing for a frame lets a
click land on a field meant to be inert.

Worth considering, not yet a task: compiling a stylesheet instead of shipping
the JIT would fix the flash of unstyled content, drop 407KB of JS from every
load, and make every dynamic class reliable. That is a build step, which
CLAUDE.md deliberately avoids, so it is Brandon's call rather than mine.
...
- 2026-08-26 · CRAFT — The nav rail reads as a void, not a machined panel · partial · Icons 24px to 16px, legends moved off mono to Archivo 10px/0.12em, all nine items are Tier 2 keys at rest with the active one lit on a plum left edge, right-edge junction added. Rail is 144px, not 128: ARRANGEMENT and THEME: DARK ellipsised at 128 and a truncated legend is not a legend. Brick framing texture still blocked with the rest of the texture work.
- 2026-08-26 · Feature — Segmented LED meter for index progress · done · 24 cells, hairline gaps, recessed unlit, neon-plum lit with the two-stop LED glow, count in mono. Zero transition on the cells, so prefers-reduced-motion has nothing to switch off. Wired into first-run setup and Health's Search Index card, which was leaking the same internal stage name setup used to. Verified: 240/445 lights 13 of 24, createMeter does not rebuild on repeat calls.
- 2026-08-26 · JS-applied Tailwind utilities · done · All six now have static homes in refrain.css, including the pointer-events-none exposure at image-crop.js:113. The refrain.css comment explaining why said "specificity"; corrected to the real cause, which is the runtime scanner never seeing a class that exists only as a JS string.
- 2026-08-26 · Alerts on a docked panel · done · DaisyUI switches .alert to a stacked centre-aligned grid below a breakpoint. On an always-docked panel the narrow case is the only case, and centred prose in a tall column reads worse, so icon stays beside the text and text ranges left at every width.
