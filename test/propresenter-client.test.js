...
test("getPlaylistItems keeps presentations and drops headers", async () => {
  ...
  await withFetch(jsonFetch({ items: [
    ...
    { type: "presentation", id: { name: "Build My Life" }, presentation_info: { presentation_uuid: "u1", arrangement_name: "Full", arrangement_uuid: "a1" } },
    ...
    assert.deepEqual(await client().getPlaylistItems("pl1"), { items: [
      { id: "u1", name: "Build My Life", arrangementName: "Full", arrangementUuid: "a1" },
      { id: "u2", name: "Untitled", arrangementName: null, arrangementUuid: null },
    ] });
  });
});
