...
**Spell check.** Pick a playlist and Refrain scans its slides for likely typos before they go up on the screen. Words it flags that are correct for your church go on an **Ignored words** list you can see and edit, so a mis-click never hides a real typo for good. It leans on the words already common across your own library, so worship vocabulary and names it has seen before don't get flagged. Refrain can't edit slides itself, so it flags each word and jumps you to the slide in ProPresenter to fix it. The same scan catches **missing media**: a background, video or image a slide points at that isn't on this Mac, named by slide, with a note when the file was saved on another Mac and never copied over. It only looks at the playlist you picked, so it's a short list you can act on, not a count of everything ever deleted.
...
- QR codes, end to end.
- Service timeline (off until `serviceModule.enabled` is set): the **Service** screen records what went live in each service, when it first went up, how long it was on screen, the gap before it, and any time you went back to it. Anything not in the service's playlist is kept and marked off-plan. It rides on what Refrain already reads to show the live readout, so it adds no traffic to ProPresenter and works with performance mode on. Times are when Refrain first saw each item, within about 4 seconds during a service. Services can be added for the day on that screen, with an optional time and playlist, or listed as a weekly schedule in `serviceModule.schedule`. Near a scheduled time Refrain checks the screens at its fast pace even with no browser open, so the times stay accurate. For events with no set time, **Lock in** watches closely and keeps performance mode on until you release it, and it survives a restart. **Before the service**, one press runs the pre-service checks on a service's playlist: ProPresenter answering, every presentation in the search index, likely typos, dates that have passed, missing media, playlist entries pointing at an arrangement the presentation no longer has (issue #5), and presentations with a twin in another library. Each check says pass, needs a look, or couldn't check, and "couldn't check" is never shown as a pass. The checks won't read ProPresenter while performance mode is on. An hour before a timed service Refrain refreshes the index on its own, and from 45 minutes out the Service screen reminds you if the checks haven't run. A **checklist** (Arrive, Before the service, Between services, After the service) can be replaced in `serviceModule.phases`. **End the day** (two presses) closes the services and any lock-in, compares the arrangements of the songs actually shown (it never pushes anything back), and writes a day summary in Markdown: the rundown per service, flags by service and type, checks that needed a look, arrangement drift, and steps not ticked. It's kept with the day's records and, if `serviceModule.summaryFolder` is set, copied there as a new file each time. Health mentions a recent day that was never ended, once. Not built yet: emailing the summary, second-device flags and a progress feed. The first two send data beyond this machine, so they wait on a deliberate decision (see `.claude/handoff.md`, section 37).
